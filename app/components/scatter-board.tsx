...
/* ── Board images data ── */
const BOARD_IMAGES = [
  { src: "/project-udhyog.jpg", top: "1%", left: "6%", rotate: "-7deg", w: 260, z: 1, side: "left" as const, startVY: 0.1, label: "UdhyogUnity — Local Business Platform" },
  { src: "/project-impulse.png", top: "8%", left: "31%", rotate: "4deg", w: 290, z: 1, side: "left" as const, startVY: 0.05, label: "Impulse IDE — AI-powered Arduino IDE" },
  { src: "/project-travelms2.jpg", top: "0%", left: "56%", rotate: "-3deg", w: 250, z: 1, side: "right" as const, startVY: 0.15, label: "TravelMS — Corporate Travel Management" },
  { src: "/project-metherium.png", top: "12%", left: "78%", rotate: "8deg", w: 275, z: 1, side: "right" as const, startVY: 0.2, label: "Metherium Studio — Creative Brand" },
  { src: "/project-macportfolio2.jpg", top: "35%", left: "3%", rotate: "6deg", w: 255, z: 1, side: "left" as const, startVY: 0.4, label: "Mac Portfolio — macOS-inspired Web Desktop" },
  { src: "/project-pokedex.jpg", top: "24%", left: "29%", rotate: "-6deg", w: 315, z: 3, side: "right" as const, startVY: 0.35, label: "Pokédex App — React + PokéAPI" },
  { src: "/ascent.jpg", top: "38%", left: "60%", rotate: "5deg", w: 305, z: 1, side: "right" as const, startVY: 0.5, label: "Ascent — College Selecting Tool" },
  { src: "/Full-Stack-Playlist.jpg", top: "68%", left: "26%", rotate: "-4deg", w: 315, z: 1, side: "right" as const, startVY: 0.7, label: "Project Playlist — Spotify-style Showcase" },
  { src: "/cursor-kit-preview.svg", top: "55%", left: "72%", rotate: "-3deg", w: 300, z: 1, side: "right" as const, startVY: 0.65, label: "CursorKit — Custom Cursor Library" },
];
...
function ScatterImage({
  ...
  return (
    <motion.div
      ...
      className={`absolute ${landed ? "cursor-grab active:cursor-grabbing" : ""}`}
      style={{
        top: img.top,
        left: img.left,
        width: img.w,
...
export function ScatterBoard({
  ...
  // Safety net for the landing above. useScroll measures this board's offsets
  // against its ancestors, but the wrapper below animates `y` and `scale` on
  // entrance — measure while that transform is still applied and the progress
  // range comes out wrong and never reaches 0.9. When that happened the tiles
  // stayed stuck at their fly-in offset and never became draggable at all.
  // Re-measuring on entrance completion (see onAnimationComplete) fixes the
  // usual case; this observer guarantees the board can't stay stuck if the
  // measurement is off for any other reason.
  useEffect(() => {
    if (landed) return;
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          timer = window.setTimeout(() => setLanded(true), 1200);
        } else if (timer) {
          clearTimeout(timer);
          timer = undefined;
        }
  ...
  return (
    <>
      <motion.div
        ...
        // The y/scale above leave a transform on this element while they run.
        // useScroll caches the board's offsets against it, so nudge a resize
        // once the transform is gone to make those measurements re-run.

...
const funFacts = [
  {
    front: "Electrical → Code",
    emoji: "⚡", image: "https://images.unsplash.com/photo-1518770660439-4636190af475?w=300&q=80&fit=crop&crop=center",
    back: "Started with a Diploma in Electrical Engineering before falling head-first into software.",
  },
  {
    front: "Sticker Boss",
    emoji: "🎨", image: "https://images.unsplash.com/photo-1558655146-9f40138edfeb?w=300&q=80&fit=crop&crop=center",
    back: "I run Metherium — a sticker & culture brand turning bold art into a community.",
  },
  {
    front: "Hackathon",
    emoji: "🏆", image: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=300&q=80&fit=crop&crop=center",
    back: "Secured 8th Rank at AceHack 5.0 — a 36-hour MLH-partnered national hackathon.",
  },
];
      ...
      >
        ...
        >
          <div className="bg-stone-100 rounded-lg flex items-center justify-center overflow-hidden" style={{ aspectRatio: "4/5" }}>
            {fact.image ? (
              <img src={fact.image} alt={fact.front} className="w-full h-full object-cover" />
            ) : (
...
const journeyData = [
  { step: "Diploma in Electrical Engineering", tag: "2020" },
  { step: "Started B.Tech in Computer Science", tag: "2023" },
  { step: "Learned the MERN stack", tag: "DEV" },
  { step: "Launched Metherium sticker brand", tag: "BUSINESS" },
  { step: "Earned AWS Cloud Practitioner cert", tag: "AWS" },
  { step: "Secured 8th Rank at AceHack 5.0", tag: "HACKATHON" },
  { step: "Full Stack Developer Intern @ Rishabh", tag: "2026" },
  { step: "Building for humans", tag: "NOW", hollow: true, pulse: true },
];
...
/* ── "Build Something" contact widget ──
   Sends a message straight to Meet's inbox. Uses Web3Forms if a key is set
   (real in-page send), otherwise falls back to opening the visitor's mail app. */
const WEB3FORMS_ACCESS_KEY = "4ea34098-e11a-479a-8f11-e1ef08486c3d";
const CONTACT_EMAIL = "meetsc04@gmail.com";

function useBuildContact() {
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          from_name: email || "Portfolio visitor",
  ...
  return { message, setMessage, email, setEmail, status, submit };
}

export function BuildSomethingWidget({ standalone = false }: { standalone?: boolean }) {
  const { message, setMessage, email, setEmail, status, submit } = useBuildContact();

  if (standalone) {
    return (
      <div className="w-full rounded-2xl border border-stone-200 bg-white/80 backdrop-blur-sm p-8 flex flex-col gap-6 shadow-sm">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[11px] text-emerald-600 font-medium uppercase tracking-wider">Ready to build</span>
          </div>
          <h2 className="font-[family-name:var(--font-noto)] text-[32px] md:text-[40px] font-black text-stone-900 leading-tight tracking-tight">Have something in mind?</h2>
          <p className="text-[15px] text-stone-500 mt-1">Pitch me an idea, a role, or a project — it lands straight in my inbox.</p>
        </div>
        <form onSubmit={submit} className="flex flex-col gap-3">
          <textarea
            ...
            rows={4}
            className="w-full resize-none rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-[14px] text-stone-700 placeholder:text-stone-400 focus:outline-none focus:border-stone-400 transition-colors"
          />
          <div className="flex items-center gap-3">
            <input
              ...
              className="flex-1 rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-[14px] text-stone-700 placeholder:text-stone-400 focus:outline-none focus:border-stone-400 transition-colors"
            />
            <button
              type="submit"
              disabled={!message.trim() || status === "sending"}
              className="shrink-0 rounded-xl bg-stone-900 text-white text-[14px] font-semibold px-6 py-3 hover:bg-stone-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              {status === "sending" ? "Sending…" : status === "sent" ? "Sent ✓" : "Send →"}
            </button>
          </div>
          {status === "sent" && <p className="text-[13px] text-emerald-600">Thanks — I'll get back to you soon!</p>}
          {status === "error" && <p className="text-[13px] text-red-500">Something went wrong. Email me at {CONTACT_EMAIL}.</p>}
        </form>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className="bg-white/90 rounded-xl p-4 flex flex-col"
    >
      <div className="flex items-center justify-between mb-1">
        <div className="text-[10px] text-stone-400 uppercase tracking-wider">Have something in mind?</div>
        <span className="text-[10px] text-emerald-600 font-medium flex items-center gap-1">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
          Ready to build
        </span>
      </div>
      <p className="text-[13px] text-stone-600 mb-3">Pitch me an idea, a role, or a project — it lands straight in my inbox.</p>
      <form onSubmit={submit} className="flex flex-col gap-2">
        <textarea
          ...
          placeholder="I've got an idea for…"
          rows={3}
          className="w-full resize-none rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-[13px] text-stone-700 placeholder:text-stone-400 focus:outline-none focus:border-stone-400 transition-colors"
        />
        <div className="flex items-center gap-2">
          <input
            type="email"
            ...
            onChange={(e) => setEmail(e.target.value)}
            placeholder="your@email.com"
            className="flex-1 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-[13px] text-stone-700 placeholder:text-stone-400 focus:outline-none focus:border-stone-400 transition-colors"
          />
          <button
            type="submit"
            disabled={!message.trim() || status === "sending"}
            className="shrink-0 rounded-lg bg-stone-800 text-white text-[13px] font-medium px-4 py-2 hover:bg-stone-900 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {status === "sending" ? "Sending…" : status === "sent" ? "Sent ✓" : "Send"}
          </button>
        </div>
        {status === "sent" && <p className="text-[12px] text-emerald-600">Thanks — I'll get back to you soon!</p>}
        {status === "error" && <p className="text-[12px] text-red-500">Something went wrong. Email me at {CONTACT_EMAIL}.</p>}
      </form>
    </motion.div>
  );
}
...
export function DesktopWidgets({ isMobile = false }: { isMobile?: boolean }) {
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      {/* Top row: Reminders, Energy, Fuel Mix */}
